...
<div class="body">
    <form id="id_calon">
        ...
        <div class="form-group">
            <label>Parent</label>
            <div class="form-line">
                <?php echo $ci->dropdown_parent_post( (!empty($dta) ? $dta->parent : null) ); ?>
            </div>
        </div>
        ...
        <div class="col-md-12">
            ...
            <div class="tab-content">
                ...
                <div role="tabpanel" class="tab-pane fade <?php echo ($key->id_bahasa=='1') ? 'in active' : '' ?>" id="kategori_<?php echo $key->id_bahasa ?>">
                    <?php $list = !empty($dta) ? $ci->get_list_post($key->id_bahasa,$dta->id_kategori_post) : '' ; ?>
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-line">
                            <input type="text" name="nama_kategori_list[<?php echo $key->id_bahasa ?>]" class="form-control input-large" value="<?php echo (empty($dta) ? '' : (!empty($list->nama_kategori_list)) ? str_replace('-', ' ', $list->nama_kategori_list) : '') ?>" />
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
...
<script type="text/javascript">
    function simpan()
    {
        $.ajax({
            type:'POST',
            url :'<?php echo base_url()."master/kategori/simpan_post/" ?>',
            ...
            success:function(i)
                ...
                if(i == 'ok')
                    ...
                    window.location.href = '<?php echo base_url() ?>master/kategori/post';
                    } else
                        {
                        alert(i);
                        }
