...
<div class="body">
    <form id="id_calon">
    	<div class="form-group">
            ...
            <div class="form-line">
                <input type="hidden" name="id_kategori" value="<?php echo (!empty($dta) ? $dta->id_kategori_galery : '') ?>">
                <input type="text" name="nama_kategori" onkeypress="if (event.which==13){simpan();}" class="form-control input-large" value="<?php echo (!empty($dta) ? ucwords(str_replace('-', ' ', $dta->nama_kategori_galery)) : '') ?>" autofocus />
            </div>
        </div>
        <div class="form-group">
            <label>Parent</label>
            <div class="form-line">
                <?php echo $ci->dropdown_parent_galery( (!empty($dta) ? $dta->parent : null) ); ?>
            </div>
        </div>
        ...
        <div class="col-md-12">
            ...
            <div class="tab-content">
                ...
                <div role="tabpanel" class="tab-pane fade <?php echo ($key->id_bahasa=='1') ? 'in active' : '' ?>" id="kategori_<?php echo $key->id_bahasa ?>">
                    <?php $list = !empty($dta) ? $ci->get_list_galery($key->id_bahasa,$dta->id_kategori_galery) : '' ; ?>
...
<script type="text/javascript">
    function simpan()
    {
        $.ajax({
            type:'POST',
            url :'<?php echo base_url()."master/kategori/simpan_galery/" ?>',
            ...
            success:function(i)
                ...
                if(i == 'ok')
                    ...
                    window.location.href = '<?php echo base_url() ?>master/kategori/galery';
                    } else
                        {
                        alert(i);
                        }
