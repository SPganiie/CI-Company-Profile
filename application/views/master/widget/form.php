...
<div class="block-header">
    <ol class="breadcrumb">
        ...
        <li><a href="<?php echo base_url('master/widget')?>"><i class="material-icons">widgets</i> Widget</a></li>
...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            <div class="header"><h2> <?php echo $ci->lang->line('tambah') ?> Widget </h2></div>
            <div class="body">
                <form id="id_calon">
                    <div class="col-md-6">
                        <div class="form-group"><label><?php echo $ci->lang->line('nama') ?> Widget</label>
                            <div class="form-line">
                                <input type="hidden" name="id_widget" value="<?php echo (!empty($dta) ? $dta->id_widget : '') ?>">
                                <input type="text" class="form-control" name="nama_utama_widget" value="<?php echo empty($dta) ? '' : $dta->nama_widget ?>" autofocus/>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group"><label><?php echo $ci->lang->line('gambar') ?> Widget</label>
                            <div class="form-line">
                                <div class="input-append">
                                    ...
                                    <input type="text" name="gambar" id="field_gambar" value="<?php echo (!empty($dta) ? $dta->gambar_widget : '') ?>" class="form-control">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <ul class="nav nav-tabs" role="tablist">
                            <?php foreach ($bhs as $key) : ?>
                                <li role="presentation" <?php echo ($key->id_bahasa=='1') ? 'class="active"' : '' ?> >
                                    <a href="#widget_<?php echo $key->id_bahasa ?>" data-toggle="tab"><?php echo ucwords($key->bahasa) ?></a>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                        <div class="tab-content">
                            <?php foreach ($bhs as $key) : ?> 
                            <div role="tabpanel" class="tab-pane fade <?php echo ($key->id_bahasa=='1') ? 'in active' : '' ?>" id="widget_<?php echo $key->id_bahasa ?>">
                                <?php $list = !empty($dta) ? $ci->get_list($key->id_bahasa,$dta->id_widget) : '' ; ?>
                                <input type="hidden" name="id_bhs[<?php echo $key->id_bahasa ?>]" value="<?php echo $key->id_bahasa ?>">

                                <div class="form-group"><label>Translate <?php echo $ci->lang->line('nama') ?> Widget</label>
                                    <div class="form-line">
                                        <input type="text" class="form-control" name="nama_widget[<?php echo $key->id_bahasa ?>]" value="<?php echo empty($dta) ? '' : (!empty($list->nama_widget_list)) ? $list->nama_widget_list : '' ?>"/>
                                    </div>
                                </div>
                                <div class="form-group"><label><?php echo $ci->lang->line('konten') ?></label>
                                    <textarea id="myeditor" name="konten[<?php echo $key->id_bahasa ?>]" class="myeditor" style="height:35%"><?php echo empty($dta) ? '' : (!empty($list->konten_widget)) ? $list->konten_widget : '' ?></textarea>
                                </div>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <a href="<?php echo base_url() ?>master/widget" class="btn btn-danger waves-effect"><?php echo strtoupper($ci->lang->line('batal')) ?></a>
...
<script type="text/javascript">
    ...
    function simpan()
    {
        if(confirm('<?php echo $ci->lang->line("yakin") ?>?'))
            ...
            $.ajax({
                type:'POST',
                url :'<?php echo base_url()."master/widget/simpan/" ?>',
                ...
                success:function(i)
                    ...
                    if(i == 'ok')
                        ...
                        window.location.href = '<?php echo base_url() ?>master/widget/';
                        } else
                            {
                            alert(i);
                            }
