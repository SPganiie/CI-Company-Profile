...
if (USE_ACCESS_KEYS == TRUE){
	...
	if (!isset($_GET['akey'], $access_keys) || empty($access_keys) || empty($_COOKIE['ci_session'])){
		die('Access Denied!');
	}

	$_GET['akey'] = strip_tags(preg_replace( "/[^a-zA-Z0-9\._-]/", '', $_GET['akey']));

	if (!in_array($_GET['akey'], $access_keys, $_COOKIE['ci_session'])){
		die('Access Denied!');
	}
}
	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
			...
			foreach ($files as $nu=>$file_array) {
				...
				$mydb = include '../../../application/config/database.php';
				$db = $db['default'];
				...
				$db_width  	= $get->width_tumbs;
				$db_height 	= $get->height_tumbs;
				...
				if(in_array($file_array['extension'], $ext_img)){
					...
					if($ftp){
						...
						if(!file_exists($src_thumb) ){						/*ukuran asli thumbs 200, 91*/
							if(!create_img($file_path, $creation_thumb_path, $db_width, $db_height,'crop',$ftp,$config)){ 
								$src_thumb=$mini_src="";
							}else{
						...
						if($img_width<$db_width && $img_height<$db_height){ /*ukuran asli thumbs 200, 91*/
							$src_thumb=$file_path;
							$show_original=true;
						}
